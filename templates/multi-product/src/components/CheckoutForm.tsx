...
export default function CheckoutForm({
  ...
  const discountPercent =
    compareTotal && savings > 0
      ...
      : 0;
  const hasSavings = discountPercent > 0 && savings > 0;
  ...
  const orderSummaryPanel = (
    <section
      className={
        isOrderSummaryOpen
          ? "summary-panel checkout-order-panel rounded-brand is-open"
          : "summary-panel checkout-order-panel rounded-brand"
      }
    >
      <button
        type="button"
      ...
      >
        <span className="checkout-order-toggle-copy">
          ...
          <span className="summary-product">
            ...
            <span className="summary-product-copy">
              ...
              {hasSavings ? (
                <span className="summary-product-saving">
                  <span>Limited offer</span>
                  <strong>Save {formatMoney(savings, product.currency)}</strong>
                </span>
              ) : null}
            </span>
          </span>
        </span>
        <span className="checkout-order-toggle-meta">
          <span className="checkout-order-total" aria-label={`Order total ${formatMoney(lineTotal, product.currency)}`}>
            <span>Total</span>
            {hasSavings ? (
      ...
      <div id="checkout-order-details" className="checkout-order-details">
        <p className="summary-sku">SKU {variant.sku}</p>
        {hasSavings ? (
